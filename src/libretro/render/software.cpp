...
void MelonDsDs::SoftwareRenderState::Render(
    ...
    void* topFramebuffer = nullptr;
    void* bottomFramebuffer = nullptr;
    bool framebuffersAvailable = nds.GPU.GetFramebuffers(&topFramebuffer, &bottomFramebuffer);
    retro_assert(framebuffersAvailable && topFramebuffer != nullptr && bottomFramebuffer != nullptr);
    if (!framebuffersAvailable || topFramebuffer == nullptr || bottomFramebuffer == nullptr) {
        return;
    }

    const auto* topScreenBuffer = static_cast<const uint32_t*>(topFramebuffer);
    const auto* bottomScreenBuffer = static_cast<const uint32_t*>(bottomFramebuffer);
    CombineScreens(

...
namespace MelonDsDs {
    ...
    struct NdsCreationArgs {
        melonDS::NDSArgs Args;
        unique_ptr<melonDS::NDSCart::CartCommon> NDSROM;
        unique_ptr<melonDS::GBACart::CartCommon> GBAROM;
    };

    struct DSiCreationArgs {
        melonDS::DSiArgs Args;
        unique_ptr<melonDS::NDSCart::CartCommon> NDSROM;
    };

    static NdsCreationArgs GetNdsArgs(
    ...
    static DSiCreationArgs GetDSiArgs(const CoreConfig& config, const retro::GameInfo* ndsInfo);
...
std::unique_ptr<melonDS::NDS> MelonDsDs::CreateConsole(
    ...
    if (type == ConsoleType::DSi) {
        ...
        DSiCreationArgs creation = GetDSiArgs(config, ndsInfo);
        auto console = std::make_unique<melonDS::DSi>(std::move(creation.Args), &state);
        console->SetNDSCart(std::move(creation.NDSROM));
        return console;
    }
    else {
        // If we're in DS mode...
        NdsCreationArgs creation = GetNdsArgs(config, ndsInfo, gbaInfo, gbaSaveInfo, state);
        auto console = std::make_unique<melonDS::NDS>(std::move(creation.Args), &state);
        console->SetNDSCart(std::move(creation.NDSROM));
        console->SetGBACart(std::move(creation.GBAROM));
        return console;
    }
}
...
static MelonDsDs::NdsCreationArgs MelonDsDs::GetNdsArgs(
    const CoreConfig& config,
    const retro::GameInfo* ndsInfo,
    const retro::GameInfo* gbaInfo,
    const retro::GameInfo* gbaSaveInfo,
    ...
    if (bios7Loaded && bios9Loaded) {
        ...
        ndsargs.ARM9BIOS = std::make_unique<melonDS::ARM9BIOSImage>(melonDS::FreeBIOSGetNtrArm9());
    ...
    unique_ptr<melonDS::NDSCart::CartCommon> ndsRom;
    if (ndsInfo) {
        ndsRom = LoadNdsCart(config, *ndsInfo);
        const uint8_t* romdata = ndsRom->GetROM();
        const NDSHeader &header = ndsRom->GetHeader();
    ...
    return {std::move(ndsargs), std::move(ndsRom), std::move(gbaRom)};
}

static MelonDsDs::DSiCreationArgs MelonDsDs::GetDSiArgs(const CoreConfig& config, const retro::GameInfo* ndsInfo) {
    ZoneScopedN(TracyFunction);
    ...
    if (firmware->GetHeader().ConsoleType != Firmware::FirmwareConsoleType::DSi) {
        ...
        throw wrong_firmware_type_exception(config.DsiFirmwarePath(), ConsoleType::DSi,
                                            static_cast<Firmware::FirmwareConsoleType>(firmware->GetHeader().ConsoleType));
    }
    ...
    dsiargs.ARM9BIOS = std::move(arm9);
    dsiargs.ARM7BIOS = std::move(arm7);
    dsiargs.Firmware = std::move(*firmware);
    dsiargs.ARM9iBIOS = std::move(arm9i);
    dsiargs.ARM7iBIOS = std::move(arm7i);
    dsiargs.NANDImage = std::move(nand);
    dsiargs.DSiSDCard = LoadDSiSDCardImage(config);

    ApplyCommonArgs(config, dsiargs);

    return {std::move(dsiargs), std::move(ndsRom)};
}
...
static void MelonDsDs::ApplyCommonArgs(const CoreConfig& config, melonDS::NDSArgs& args) noexcept {
    ...
    args.OutputSampleRate = SAMPLE_RATE;
#ifdef JIT_ENABLED
    if (config.JitEnable()) {
        args.JIT = {

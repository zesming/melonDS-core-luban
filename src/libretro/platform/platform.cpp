...
void Platform::Mic_Start(void* userdata) {
}

void Platform::Mic_Stop(void* userdata) {
}

int Platform::Mic_ReadInput(s16* data, int maxlength, void* userdata) {
    if (data == nullptr || maxlength <= 0 || userdata == nullptr) {
        return 0;
    }

    auto& core = *static_cast<MelonDsDs::CoreState*>(userdata);
    core.ReadMicInput(std::span<int16_t>(data, static_cast<size_t>(maxlength)));
    return maxlength;
}
...
bool Platform::Addon_KeyDown(KeyType type, void* userdata) {
    return false;
}

float Platform::Addon_MotionQuery(MotionQueryType type, void* userdata) {
    return 0.0f;
}
